...
def motifplot(G, proatom_order, lig_name, MotifFolder=motifFolder):
    '''this function is to integrate bar and pie charts into one picture, and make a venn plot to show
       overlaps of protein pockets containing different motifs for one ligand
       o input: (1) G: the number of function groups of the ligand , type: int
                ...
                (4) MotifFolder: the location of a folder to place plots that display motif features for all ligands
...
# # plot the bar, pie chart for ligands comprising 2 or more function groups, plot the venn diagrams for the
# # ligands comprising 2 or 3 function groups.
# df = pd.read_excel(ligInfo, index_col=0)
# for i in df.index:
#     g_num = df.loc[i]["Number"]
#     if g_num != 1:
#         with open(os.path.join(statisFolder, i), "r") as f:
#             pro_atom = json.loads(f.readlines()[0])["pro"]
#             motifplot(g_num, pro_atom, i)


##################the Reusability analysis for Adenine,Ribose,Triphosphate of ATP#####################
...
#########################the Reusability analysis for overall function groups##########################
...
fig.savefig(os.path.join(Workdir, "aftme", "PPC_overall.png"), dpi=300, bbox_inches="tight")



#########################the Reusability calculation for function groups which count >=3##########################
