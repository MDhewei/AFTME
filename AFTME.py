...
def motifGen(LigInfo, DistFolder, HeatFolder, StatisFolder, ScoreFolder):
    '''this function is to obtain the clustering heatmap, corresponding clustering result for each of all ligands
    ...
    for ligand in df_info.index:
        ...
        g_num = df_info.loc[ligand]["Functional Group Number"]
        ...
        plt.savefig(os.path.join(HeatFolder, ligand + "_cls.png"), dpi=300, bbox_inches='tight')
        try:
...
def assign_score(AtomsLigand, StatisFolder, AssignFolder, ScoreFolder):
    '''this function is to obtain assignment between artificially defined function groups and clustered function groups
       ...
       o input: (1) AtomsLigand: the location of excel file recording inherent atom composition for each manually
                ...
                (2) StatisFolder: the location of the folder placing txt files recording clustered atoms of function group
                                  and binding motif for each of 233 ligands.
                (3) AssignFolder: the location of folder placing an excel file recording assignment between artificially
    ...
    for ligand in atom_lib.index:
        ...
        # add ligands in which the function groups count=1, whose quality score=1
            ...
            score2nd["scores"].append([(fname, 1.0)])

    # write the assignment and score into corresponding excel files
    pd.DataFrame(assign).to_excel(os.path.join(AssignFolder, "assign.xlsx"), index=False)
    pd.DataFrame(score2nd).to_excel(os.path.join(ScoreFolder, "score2nd.xlsx"), index=False)


if __name__ == "__main__":
    # Add aruguments for user input with command lines
    parser = argparse.ArgumentParser(description="motif extraction and scoring")
    subparsers = parser.add_subparsers(dest="subparser_name")


    # Add subparser: motifGen
    parser_motif = subparsers.add_parser("motifGen", help="obtain the clustering heatmap, corresponding "
                                                          "clustering result for each of all ligands and the "
                                                          "first stage scores for each of the ligands comprising "
                                                          "2 or more function groups")
    parser_motif.add_argument("-i", "--inputfile", type=str, nargs=2, required=True, help="2 files in order: "
                                                                                          "an excel file recording all "
                                                                                          "233 ligands information, and"
                                                                                          "a folder placing distance "
                                                                                          "matrix excel files created "
                                                                                          "for each of 233 ligands")
    parser_motif.add_argument("-o", "--outfile", type=str, nargs=3, required=True, help="3 files in order:"
                                                                                        "a folder for saving clustering 233 heatmaps of the 233 ligands;"
                                                                                        "a folder for saving clustering result texts recording atoms of function group and binding motifs;"
                                                                                        "a folder for saving excel file recording the first stage scores of 181 ligands with 2 or more funtion groups out of 233 ligands")


    # Add subparser: assign_score
    parser_assign = subparsers.add_parser("assign_score", help="obtain assignment between artificially defined function "
                                                               "groups and clustered function groups of each of 233 ligands, "
                                                               "and all ligands scores based on the first stage scores")
    parser_assign.add_argument("-i", "--inputfile", type=str, nargs=3, required=True, help="the 3 files in order:"
                                                                                         "an excel file recording inherent atom composition for each manualy predefined function group of each of 233 ligands;"
                                                                                         "a folder placing txt files recording clustered atoms of function group and binding motif for each of 233 ligands, which is genetated from motifGen"
                                                                                         "a folder placing excel files recording 1st stage scores, which is generated from motifGen")

    parser_assign.add_argument("-o", "--outfile", type=str, required=True, help="the 2 files in order: a folder for saving "
                                                                                "an excel file recording assignment between "
                                                                                "artificially defined function groups and "
                                                                                "clustered function groups of each of 233 ligands")


    # obtain parameters from user input with commandline
    args = parser.parse_args()

    if args.subparser_name == "motifGen":
        ligInfo, distFolder = args.inputfile
        heatFolder, statisFolder, scoreFolder = args.outfile
        motifGen(LigInfo=ligInfo, DistFolder=distFolder, HeatFolder=heatFolder, StatisFolder=statisFolder,
                 ScoreFolder=scoreFolder)


    if args.subparser_name == "assign_score":
        atomsLigand, statisFolder, scoreFolder = args.inputfile
        assignFolder = args.outfile
        assign_score(AtomsLigand=atomsLigand, StatisFolder=statisFolder, ScoreFolder=scoreFolder,
